...
async def check_payment(rental_id):
        if postgres.perform_get_query("get_latest_payment_status", rental_id)[0][0] == "approved":
            return postgres.perform_get_query("get_rental_current_expiration_date", rental_id)[0][0]
        ...
        if response.get("status") == "approved":
            time = postgres.perform_insert_or_update_returning_query("update_payment_status", (response.get("status"), int(rental_id), int(rental_id)))
            expiration_date = postgres.perform_insert_or_update_returning_query("update_rental_expiration_first", (int(time), int(rental_id), int(rental_id)))
...
async def create_time_addition_payment(rental_id, time_rented):
    ...
    if status_code not in [201, 200]:
        raise HTTPException(status_code=400, detail="error on payment_creation")
    postgres.perform_insert_or_update_query("create_payment", (
...
async def payment_updated_webhook(request: Request):
    ...
    if payment_status == "approved":
        sse.publish({"status" : "approved", "parasun_id": parasun_id, "expiration_date":expiration_date}, type=rental_id)
